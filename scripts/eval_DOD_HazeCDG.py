#!/usr/bin/env python3
"""
DOD (Diffusion Once and Done, AAAI 2026) runner with Haze Counter-Degradation
Guidance (HazeCDG).

Intended project location
-------------------------
    scripts/eval_DOD_HazeCDG.py

Method summary
--------------
DOD is an independently trained one-step diffusion restorer. HazeCDG reuses the
frozen LHD Stage1 HazeGen only as a degradation-sensing model; DOD itself is
not retrained or adapted.

At DOD's native latent state and timestep t=999, the haze differential is
measured using the frozen HazeGen branch and its frozen backbone:

    g = eps_D - eps_base,

where eps_D is the prediction with the learned HazeGen ControlNet residuals and
eps_base is the prediction of the same frozen SD2.1 backbone without those
residuals. Reversing this local degradation change defines the
counter-degradation reference

    eps_CD = eps_base - g.

As in the main HazeCDG formulation, preserving the relative prediction offset
between the restoration and degradation predictions gives the
counter-degradation transport

    eps_R -> eps_R - 2 g.

DOD, however, performs the entire restoration in a single reverse transition.
Following the paper's one-step implementation, we therefore match the
cumulative correction energy of the 50-step reference sampler using a factor
computed only from the known diffusion schedules:
...
eta_DOD is schedule-derived and is not tuned on validation data. The corrected
prediction is passed to DOD's original scheduler step, followed by the released
Stage2 HDE decoder. All DOD and HazeGen model parameters remain frozen.
...
----------------
    ...
    hazecdg
        ...
        inserted immediately before DOD's scheduler step.

One-step setting
----------------
DOD performs exactly one restoration UNet evaluation at native timestep t=999.
HazeCDG therefore performs one degradation-sensing operation (K=1). Sparse
anchor scheduling and causal reuse are not applicable to DOD.

Haze sensing
------------
For hazecdg mode only, the released LHD Stage1 HazeGen is loaded as a frozen
sensing model. At DOD's exact lq_latent and t=999:
...
The zero latent hint is used for HazeGen sensing, consistent with the real-image
HazeGen setting. The DOD latent is not re-encoded by LHD; it is queried directly
in the LHD Stage1 epsilon field.

Supported evaluation datasets
-----------------------------
Only the three real-world haze benchmarks evaluated in the paper are supported:
...
paper protocol, place the 4,322 RTTS images, the paper's 150-image URHI subset,
...
These checks guard against silent prediction-parameterization or diffusion-
schedule mismatches.
...
then cropped back to the original DOD latent HxW before constructing g. Thus,
...
# One-step schedule normalization from Appendix B of the paper. This value is
# determined from the DOD and 50-step reference diffusion schedules, not tuned
# on validation data.
DOD_HAZECDG_ETA = 0.44341345
...
def project_root_from_script() -> Path:
    # scripts/eval_DOD_HazeCDG.py -> project root
...
# =============================================================================
# One-step HazeCDG guidance core
# =============================================================================
...
def _guidance_common_diagnostics(
    ...
    g = haze_differential.detach().float().reshape(-1)
    raw = raw_transport.detach().float().reshape(-1)
    applied = applied_correction.detach().float().reshape(-1)

    return {
        "eps_r_norm": float(torch.linalg.vector_norm(r).item()),
        "g_norm": float(torch.linalg.vector_norm(g).item()),
        "raw_transport_norm": float(torch.linalg.vector_norm(raw).item()),
        "applied_correction_norm": float(torch.linalg.vector_norm(applied).item()),
        "eta_dod": float(DOD_HAZECDG_ETA),
    }
...
def apply_counter_degradation_guidance(
    ...
    """Apply the paper's one-step DOD HazeCDG update.

    The haze differential is

        g = eps_D - eps_base.

    Reversing the degradation reference from eps_base + g to eps_base - g
    gives the parameter-free counter-degradation transport 2g. Because DOD
    performs restoration in a single reverse transition, Appendix B matches
    the cumulative correction energy of the 50-step reference sampler using
    the schedule-derived factor eta_DOD:

        eps_R^HazeCDG = eps_R - 2 * eta_DOD * g.

    eta_DOD is determined only from the known diffusion schedules and is not a
    manually tuned guidance-strength hyperparameter.
    """
    ...
    raw_transport = 2.0 * g
    applied_correction = DOD_HAZECDG_ETA * raw_transport
    guided = (eps_r.float() - applied_correction).to(dtype=eps_r.dtype)
...
def verify_xformers_runtime(device: str) -> None:
    """Verify the FP16 xFormers path used by the Blackwell compatibility shim."""
    try:
        import xformers
    ...
    try:
        # Match LHD's transformer head dimension (64), but use FP16 because
        # Blackwell xFormers FA kernels do not accept the LHD FP32 Q/K/V path.
        q = torch.randn(5, 64, 64, device=dev, dtype=torch.float16)
        ...
        raise RuntimeError(
            "xFormers FP16 memory-efficient attention failed on the active GPU.\n"
    ...
    print(f"[env] xformers          : {xformers.__version__} (FP16 preflight PASS)")
    del q, k, v, y
...
def import_dod(repo_root: Path):
    ...
    # When this runner is launched from scripts/, any unrelated regular
...
def patch_lhd_xformers_attention_dtype() -> None:
    """
    Keep the cloned LHD repository unchanged while making its xFormers
    cross-attention compatible with Blackwell.

    LHD keeps the Stage1 sensing UNet/ControlNet in FP32 and therefore produces
    FP32 Q/K/V tensors. On the active Blackwell xFormers path, the available
    fused attention kernels accept FP16/BF16 rather than FP32. Cast only Q/K/V
    for the xFormers call, then restore the attention output to the original
    dtype before the output projection. All model parameters and the surrounding
    LHD sensing computation remain unchanged.
    """
    ...
    if getattr(cls, "_hazecdg_blackwell_dtype_patched", False):
        return
    ...
    cls._hazecdg_blackwell_dtype_patched = True
    print("[env] LHD xFormers QKV  : fp16 (output restored to FP32)")
...
class LHDStage1Sensor:
    ...
    def predict_degradation_and_base(
        ...
        # Keep the frozen LHD Stage1 sensing model/state in FP32. DOD's fp16
        # latent is cast only for the sensor; no re-encoding occurs. When the
        # xFormers backend is selected, the Blackwell compatibility shim casts
        # only attention Q/K/V tensors to FP16 inside the fused attention call.
...
def dod_one_step_forward(
    ...
    *,
    ...
    # HazeCDG : eps_r -> eps_r - 2*eta_DOD*(eps_D - eps_base) -> sched.step(...)
    # ======================================================================
...
class DODRunner:
    ...
    def _run_one(self, image_path: Path, image_seed: int):
        ...
        if diag is not None and self.cfg.log_diagnostics:
            print(
                ...
                f" | eta_DOD={diag['eta_dod']:.8f}"
                f" | ||eps_R||={diag['eps_r_norm']:.4e}"
                f" | ||g||={diag['g_norm']:.4e}"
                f" | ||2g||={diag['raw_transport_norm']:.4e}"
                f" | ||2*eta_DOD*g||={diag['applied_correction_norm']:.4e}"
                f" | corr/eps_R={diag['applied_correction_norm'] / max(diag['eps_r_norm'], NUMERICAL_EPS):.4f}"
            )
    ...
    def run(self) -> None:
        ...
        print("=" * 108)
        print("DOD :: official one-step restoration + Haze Counter-Degradation Guidance")
        print("=" * 108)
        ...
        print(f"Seed policy  : constant seed={self.cfg.seed} reset for every image")
        ...
        if self.cfg.mode == "hazecdg":
            ...
            print("HazeCDG calls   : 1 degradation-sensing operation before DOD sched.step")
            print("Sparse reuse : N/A (DOD has one restoration step)")
            print(f"eta_DOD      : {DOD_HAZECDG_ETA:.8f} (schedule-derived)")
            print("Guidance rule: eps_R - 2*eta_DOD*g")
        print("=" * 108)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "DOD official one-step baseline / Haze Counter-Degradation Guidance."
        )
    )
    parser.add_argument(
        "--config",
    ...
    if args.check_only:
        ...
        if runtime_cfg.mode == "hazecdg":
            print(f"[check] eta_DOD         : {DOD_HAZECDG_ETA:.8f} (schedule-derived)")
            print("[check] HazeCDG rule    : eps_R - 2*eta_DOD*g")
            print("[check] sensing calls   : exactly 1")
            print("[check] insertion       : after DOD model_pred, before sched.step")
            print("[check] sparse reuse    : N/A for one-step DOD")
