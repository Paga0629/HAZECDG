...
-------------------------
    DFG/scripts/eval_LHD_HazeCDG.py

HazeCDG at a sensing anchor
---------------------------
Let eps_R denote the ordinary DiffDehaze (Stage2) epsilon prediction. HazeCDG
reuses the frozen learned haze-generation branch to identify a local haze
differential at the same diffusion state:

    g_a = eps_D,a - eps_base,a

where eps_D,a is the haze-conditioned prediction and eps_base,a is the same
shared diffusion model evaluated without the haze ControlNet residual.
Therefore the learned degradation prediction can be written as

    eps_D,a = eps_base,a + g_a.

Reflecting this learned degradation displacement about eps_base,a gives its
counter-degradation counterpart

    eps_C,a = eps_base,a - g_a.

The displacement from the learned degradation prediction to this symmetric
counter-degradation counterpart is exactly

    eps_D,a - eps_C,a = 2 g_a.

HazeCDG transfers this parameter-free counter-degradation displacement to the
restoration prediction:

    q_a = 2 g_a
    eps_guided,a = eps_R,a - q_a.

The factor 2 is not tuned from data. It follows directly from the symmetric
mapping +g_a -> -g_a around the frozen-backbone prediction.

Sparse sensing
--------------
The degradation/base branch is evaluated only at K causal sensing anchors.
The anchors are uniformly distributed in reverse-call index and use a nested
hierarchy for the standard 50-step evaluation:

    A5 subset A10 subset A25 subset A50.

At an anchor a, HazeCDG computes q_a exactly. At every following non-anchor
step k before the next anchor, it directly reuses the most recent epsilon-space
correction:

    q_k = q_a.

Thus non-anchor steps require only the ordinary Stage2 eps_R prediction and one
subtraction; they perform no additional degradation/base forward.

Dense consistency
-----------------
If K equals the number of reverse evaluations, every step is an anchor and the
counter-degradation transport is recomputed at every step. Smaller K values use
causal zero-order holding of the most recently sensed epsilon correction. No
future anchor is used.

Controlled modes
----------------
    baseline
        Official LHD Stage2 restoration with the standard SpacedSampler.
        Stage1 HazeGen is not loaded.

    hazecdg
        The same Stage2 trajectory with HazeCDG sensing at K anchors and direct
        epsilon-correction holding between anchors. Multiple K values can be
        swept from YAML via

            hazecdg.anchor_counts: [5, 10, 25, 50]

Implementation scope
--------------------
- G_R and G_D are frozen; no model parameter is updated.
- HazeCDG has no tuned guidance-strength hyperparameter.
- Standard LHD SpacedSampler is used; AccSamp / RGB fidelity guidance are off.
- Stage1 uses the validated zero image hint at sensing anchors only.

Preprocessing
-------------
The controlled LHD preprocessing is preserved:
...
4) output is resized back to the original HxW before saving.
...
def project_root_from_script() -> Path:
    # DFG/scripts/eval_LHD_HazeCDG.py -> DFG/
...
def _guidance_common_diagnostics(
    eps_r: torch.Tensor,
    degradation_direction: torch.Tensor,
    correction: torch.Tensor,
) -> Dict[str, float]:
    r = eps_r.detach().float().reshape(-1)
    g = degradation_direction.detach().float().reshape(-1)
    q = correction.detach().float().reshape(-1)

    return {
        "eps_r_norm": float(torch.linalg.vector_norm(r).item()),
        "g_norm": float(torch.linalg.vector_norm(g).item()),
        "correction_norm": float(torch.linalg.vector_norm(q).item()),
    }
...
def apply_hazecdg_guidance(
    ...
    *,
    ...
    """Apply the parameter-free HazeCDG counter-degradation transport.

    The learned haze differential is

        g_t = eps_D - eps_base.

    Relative to the frozen-backbone prediction, eps_D lies at +g_t. Its
    symmetric counter-degradation counterpart lies at -g_t. The displacement
    from +g_t to -g_t is therefore exactly 2 g_t, giving

        q_t = 2 g_t,
        eps_guided = eps_R - q_t.

    The factor 2 is determined by this symmetry and is not estimated from the
    restoration prediction or tuned from a dataset.
    """
    if eps_r.shape != degradation_direction.shape:
        raise ValueError(
            "HazeCDG guidance shape mismatch: "
            f"eps_r={tuple(eps_r.shape)}, "
            f"g={tuple(degradation_direction.shape)}"
        )

    g = degradation_direction.detach().float()
...
def choose_anchor_eval_indices(
    *,
    ...
    """Choose uniform nested causal sensing anchors over the reverse calls.

    For the standard 50-step HazeCDG evaluation, we use a nested hierarchy

        A5 ⊂ A10 ⊂ A25 ⊂ A50

    so increasing K only adds new sensing anchors while preserving all anchors
    used by the smaller-K setting.

    The anchor locations are predefined, deterministic, and dataset-independent.
    """
    ...
    # ------------------------------------------------------------------
    # Canonical uniform nested schedules for the 50-step HazeCDG experiments.
    #
    # A5 ⊂ A10 ⊂ A25 ⊂ A50
    #
...
def verify_xformers_runtime(device: str) -> None:
    if not str(device).startswith("cuda"):
        raise RuntimeError("runtime.attention_mode=xformers requires a CUDA device.")

    try:
        import xformers
        import xformers.ops as xops
        import xformers.ops.fmha.cutlass as cutlass
    except Exception as exc:
        raise RuntimeError(
            "runtime.attention_mode=xformers but xformers could not be imported."
        ) from exc

    dev = torch.device(device)
    index = 0 if dev.index is None else int(dev.index)
    capability = tuple(torch.cuda.get_device_capability(index))
    max_capability = tuple(cutlass.FwOp.CUDA_MAXIMUM_COMPUTE_CAPABILITY)

    print(f"[env] xformers          : {xformers.__version__}")
    print(f"[env] GPU capability    : {capability}")
    print(f"[env] xformers max cc    : {max_capability}")

    if capability > max_capability:
        raise RuntimeError(
            "The active xformers environment still blocks this GPU capability.\n"
            f"GPU capability={capability}, xformers maximum={max_capability}."
        )

    tests = [
        ("UNet-like FP32", (1, 256, 5, 64), torch.float32),
        ("VAE-like FP32", (1, 64, 1, 512), torch.float32),
    ]
    for label, shape, dtype in tests:
        try:
            ...
            print(f"[env] xformers preflight : {label} PASS")
            del q, k, v, y
        except Exception as exc:
            raise RuntimeError(
                f"xformers preflight failed for {label}, shape={shape}, dtype={dtype}."
            ) from exc
...
def make_sparse_guidance_sampler_class(OfficialSpacedSampler):
    ...
    class SparseHazeCDGSampler(OfficialSpacedSampler):
        ...
        def reset_image_state(self) -> None:
            ...
            self.query_count = 0
            self.guided_count = 0
            self.held_step_count = 0
            self.active_count = 0

            self.held_anchor_correction: Optional[torch.Tensor] = None
            self.held_anchor_call_index: Optional[int] = None

            self.sum_anchor_g_norm = 0.0
            self.sum_anchor_eps_r_norm = 0.0
        ...
        def _accumulate_anchor_diag(self, diag: Dict[str, float]) -> None:
            ...
            self.sum_anchor_eps_r_norm += diag["eps_r_norm"]
        ...
        def _maybe_log_application(
            ...
            *,
            ...
            if is_anchor and diag is not None:
                ...
                extra = (
                    f" | anchor=0"
                    f" | held_from={self.held_anchor_call_index + 1 if self.held_anchor_call_index is not None else -1}"
                    f" | q_k=q_a"
                )
            print(
        ...
        def p_sample(
            ...
            if is_anchor:
                # 2a) Sense the local haze differential and construct the
                #     symmetric counter-degradation transport:
                #
                #         g_a    = eps_D,a - eps_base,a
                #         q_a    = 2 g_a
                #         eps'_a = eps_R,a - q_a
                #
                #     Relative to eps_base,a, the learned degradation branch lies
                #     at +g_a and its mirror counterpart lies at -g_a. Moving from
                #     +g_a to -g_a therefore yields the parameter-free 2g_a
                #     transport used by HazeCDG.
                ...
                self.held_anchor_correction = anchor_correction
                self.held_anchor_call_index = self.call_index
                correction = anchor_correction
                self._accumulate_anchor_diag(diag)
                del eps_deg, eps_base, g_t
            else:
                # 2b) Sparse reuse. No G_D or eps_base is recomputed. The most
                #     recently sensed epsilon correction
                #     is held causally until the next anchor:
                #
                #         q_k = q_a.
                if (
                    self.held_anchor_correction is None
                    or self.held_anchor_call_index is None
                ):
                    raise RuntimeError(
                        "Sparse HazeCDG reached a non-anchor before the first sensing anchor."
                    )

                correction = self.held_anchor_correction
                eps_guided = (
                    eps_r.float() - correction
                ).reshape_as(eps_r.float()).to(dtype=eps_r.dtype)
                self.held_step_count += 1

            corr_norm = float(torch.linalg.vector_norm(correction.float()).item())
            self.guided_count += 1
            self.active_count += int(corr_norm > NUMERICAL_EPS)
...
class LHDRunner:
    ...
    def _load_models(self) -> None:
        ...
        print("[HazeCDG] anchor transport   : q_t = 2 g_t")
        print("[HazeCDG] geometric meaning  : +g_t -> -g_t around eps_base")
        print("[HazeCDG] sparse reuse       : q_k = q_a between anchors")
    ...
    def _build_sampler(self) -> None:
        ...
        print("[sampler] HazeCDG : standard SpacedSampler + symmetric counter-degradation transport + epsilon hold")
        ...
        *,
        ...
        if self.cfg.mode == "hazecdg":
            ...
            print(
                ...
                f" | held steps={self.sampler.held_step_count}"
                f" | active={100.0*self.sampler.active_count/gn:.1f}%"
    ...
    def run(self) -> None:
        ...
        print("=" * 108)
        print("HazeCDG :: symmetric counter-degradation transport + sparse epsilon-correction hold")
        print("=" * 108)
...
def build_runtime_config(
    ...
    raw_anchor_counts = nested_get(
        cfg_dict, ["hazecdg", "anchor_counts"], [5, 10, 25, 50]
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "LHD baseline / HazeCDG symmetric counter-degradation transport with sparse epsilon holding."
        )
    )
    parser.add_argument(
        "--config",
        ...
        if runtime_cfg.mode == "hazecdg":
            ...
            for k in runtime_cfg.anchor_counts:
                anchors = choose_anchor_eval_indices(
                    steps=runtime_cfg.steps, anchor_count=k
                )
                print(f"[check] K={k:<3d} calls : {anchors}")
            print("[check] non-anchor   : q_k=q_a direct epsilon hold")
            print("[check] K=steps      : dense per-step HazeCDG transport")
